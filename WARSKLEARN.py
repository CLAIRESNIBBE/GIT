...
def main():
    ...
    modelsk_GBT = ensemble.AdaBoostRegressor(tree.DecisionTreeRegressor(max_depth=4),n_estimators=300, random_state=rng)
    ...
    for file in filesImp:
        ...
        stack = StackingCVRegressor(regressors=(xgbr, rf, modelsk_LR), meta_regressor=xgbr, use_features_in_secondary=True)
        ...
        stack4 = StackingCVRegressor(regressors=(modelsk_GBR,modelsk_NN,modelsk_RR),meta_regressor = modelsk_RR,use_features_in_secondary=True)
        stack4.fit(X_train,y_train)
        y_pred_stack4 = stack4.predict(X_test)
        stack5 = StackingCVRegressor(regressors=(SV,modelsk_GBR,modelsk_NN),meta_regressor=SV,use_features_in_secondary=True)
        ...
        for m in range(len(MLearning)):
            ...
            impResults.append({'Imp': df, 'model': skmodel, 'MAE': mae, 'PW20': pw20, 'R2': R2, 'MLAR': bias, 'MALAR': mape, 'y-true':y_test, 'y-pred':y_pred})
